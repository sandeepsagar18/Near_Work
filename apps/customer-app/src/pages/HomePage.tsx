  ...
  return (
    <div className="min-h-screen pb-24 bg-slate-50 text-gray-900">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-6 space-y-8">
        ...
        {/* Active Online Workers / Technicians Available Near You */}
          ...
          <section className="space-y-4">
            <div className="flex justify-between items-center">
              <div>
                <div className="flex items-center space-x-2">
                  ...
                  <h2 className="text-lg sm:text-xl font-black text-gray-900">Verified Experts Online Near You</h2>
                </div>
                <p className="text-xs text-gray-500 mt-0.5">Active technicians ready for instant doorstep service dispatch</p>
              </div>
              <span className="text-xs font-bold text-emerald-700 bg-emerald-50 border border-emerald-200 px-3 py-1 rounded-full">
                {activeWorkers.length} Online Now
              </span>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {activeWorkers.map((worker) => (
                <div
                  key={worker.id}
                  className="bg-white rounded-3xl p-5 border border-gray-100 shadow-sm hover:shadow-lg hover:border-emerald-200 transition-all flex items-center justify-between gap-3 group"
                >
                  <div className="flex items-center space-x-3.5 min-w-0">
                    <div className="relative">
                      <div className="w-12 h-12 rounded-2xl bg-indigo-600 flex items-center justify-center text-white font-black text-base shadow-md shadow-indigo-600/30">
                    ...
                    <div className="min-w-0">
                      ...
                      <p className="text-xs text-indigo-600 font-semibold truncate mt-0.5">
                        {worker.skills?.join(', ') || 'General Expert'}
                      </p>
                      <div className="flex items-center space-x-2 text-[11px] text-gray-400 mt-1">
                        ...
                        <span>•</span>
                        <span className="text-emerald-600 font-semibold">{worker.distanceKm} km away</span>
                      </div>
                    </div>
                  </div>
                    ...
                    onClick={() => {
                      if (worker.skills && worker.skills.length > 0) {
                        const targetCategory = categories.find((c) =>
                          worker.skills.some((s: string) => s.toLowerCase().includes(c.name.toLowerCase()))
                        );
                        if (targetCategory) setSelectedCategory(targetCategory.slug);
                      }
                    }}
                    className="px-3.5 py-2 bg-emerald-50 hover:bg-emerald-600 text-emerald-700 hover:text-white rounded-xl text-xs font-bold whitespace-nowrap transition cursor-pointer flex-shrink-0"
                  >
                    Request
                  </button>
                </div>
